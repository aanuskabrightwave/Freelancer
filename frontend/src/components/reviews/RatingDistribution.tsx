...
export default function RatingDistribution({ reviews, clean = false }: RatingDistributionProps) {
      ...
      {([5, 4, 3, 2, 1] as const).map((stars) => {
        ...
        return (
          <div key={stars} className="flex items-center gap-3 text-xs">
            <span className="w-8 text-text-sub font-semibold text-right text-xs">{stars} ★</span>
            ...
            <span className="w-14 text-right text-text-sub font-bold text-xs">
              {count} ({percent}%)
            </span>
          </div>
        );
