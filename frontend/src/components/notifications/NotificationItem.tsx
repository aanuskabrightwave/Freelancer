...
export default function NotificationItem({ notification, onMarkRead }: NotificationItemProps) {
  ...
  const getIcon = (type: string) => {
    switch (type.toUpperCase()) {
      ...
      case "REVIEW":
        return <Star className="w-5 h-5 text-pink-400" fill="currentColor" />;
      case "PAYOUT":
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={`flex items-start gap-4 p-4 border border-white/5 rounded-xl cursor-pointer transition-all duration-300 ${
        notification.is_read
          ? "bg-white/[0.02] hover:bg-white/[0.04]"
          : "bg-white/[0.07] hover:bg-white/[0.09] shadow-lg shadow-indigo-500/5"
      }`}
    >
      {/* Icon Badge */}
      <div className="flex-shrink-0 p-2.5 bg-white/5 border border-white/5 rounded-xl">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-white/60 leading-relaxed break-words line-clamp-2">
      ...
      {/* Unread Indicator */}
      {!notification.is_read && (
        <span className="flex-shrink-0 w-2.5 h-2.5 bg-indigo-500 rounded-full animate-pulse self-center" />
      )}
    </div>
  );
}
