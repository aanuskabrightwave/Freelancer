...
export default function EditFreelancerProfile() {
    ...
    async function loadData() {
      try {
        ...
        if (err.response?.status === 404) {
          // Initialize empty state for creation
          setProfileId(null);
        } else {
  ...
  const handleSaveDetails = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profileId) {
        ...
        setSuccessMsg("Profile details saved successfully. Redirecting back...");
        setTimeout(() => {
        ...
        setCompletionPercentage(res.profile_completion_percentage);
        setIsProfilePublic(res.is_profile_public);
        setSuccessMsg("Profile details created successfully. Redirecting back...");
        setTimeout(() => {
          router.push("/freelancer/profile");
        }, 1500);
      }
  ...
  const handleSaveSkills = async () => {
    try {
      ...
      if (!profileId) throw new Error("Please save your profile details first before adding skills.");
  ...
  const handleTogglePublic = async () => {
    ...
    try {
      ...
      if (!profileId) throw new Error("Please save your profile details first.");
  ...
  const handleAddEquipment = async () => {
    ...
    try {
      ...
      if (!profileId) throw new Error("Please save your profile details first before adding equipment.");
  ...
  const handleAddPortfolio = async () => {
    ...
    try {
      ...
      if (!profileId) throw new Error("Please save your profile details first before adding portfolio items.");
