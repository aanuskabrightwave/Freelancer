      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Track payments credits, platform commission calculations, and payouts transfers.</p>
          </div>
          <div className="flex items-center gap-4">
            <Link href="/freelancer/dashboard" className="text-xs uppercase tracking-widest font-black text-slate-400 hover:text-indigo-400 flex items-center gap-2 group transition">
        ...
        {summary && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
              <div className="absolute top-4 right-4 w-8 h-8 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-400 border border-indigo-500/20">
              ...
              <h3 className="text-xxs font-black uppercase tracking-widest text-slate-400">Total Earned</h3>
              <p className="text-2xl font-black text-slate-100 mt-2">₹{Number(summary.total_earned).toLocaleString("en-IN")}</p>
              <p className="text-xxs text-slate-500 font-mono mt-1">Platform Commission Deducted</p>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
              ...
              <h3 className="text-xxs font-black uppercase tracking-widest text-slate-400">Pending Clearance</h3>
              <p className="text-2xl font-black text-amber-400 mt-2">₹{Number(summary.pending).toLocaleString("en-IN")}</p>
              <p className="text-xxs text-slate-550 font-mono mt-1">Held under payouts clearance hold</p>
            </div>

            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden bg-gradient-to-br from-slate-900 to-indigo-950/20">
              ...
              <h3 className="text-xxs font-black uppercase tracking-widest text-slate-400">Available for Payout</h3>
              <p className="text-2xl font-black text-emerald-400 mt-2">₹{Number(summary.available).toLocaleString("en-IN")}</p>
              <p className="text-xxs text-slate-500 font-mono mt-1">Cleared balance transfer eligibility</p>
            </div>

            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
              <div className="absolute top-4 right-4 w-8 h-8 bg-blue-500/10 rounded-xl flex items-center justify-center text-blue-400 border border-blue-500/20">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                </svg>
              </div>
              <h3 className="text-xxs font-black uppercase tracking-widest text-slate-400">Paid Out</h3>
              <p className="text-2xl font-black text-blue-400 mt-2">₹{Number(summary.paid_out).toLocaleString("en-IN")}</p>
              <p className="text-xxs text-slate-550 font-mono mt-1">Transferred directly to bank</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col justify-between shadow-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 text-white disabled:text-slate-500 font-black text-xs py-3.5 px-6 rounded-2xl transition duration-200 shadow-lg shadow-indigo-600/10 active:scale-[0.99]"
              >
                {payoutLoading ? "Processing Transfer Request..." : "Payout Cleared Balance (Auto Transfer)"}
              </button>
              <Link
                href="/freelancer/earnings/payouts"
                className="bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white font-bold text-xs py-3.5 px-6 rounded-2xl transition duration-200 text-center"
              >
                View Payout Logs
              </Link>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col justify-between shadow-xl">
            <div>
              <div className="w-12 h-12 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-2xl flex items-center justify-center mb-6">
            ...
            <Link
              ...
              className="bg-slate-950 hover:bg-slate-800 border border-slate-800 text-indigo-400 hover:text-indigo-300 font-black text-xs py-3.5 px-6 rounded-2xl transition duration-200 text-center"
            >
              Configure Account
            </Link>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl shadow-xl overflow-hidden">
          <div className="p-6 border-b border-slate-800/80 flex items-center justify-between">
            ...
            <Link href="/freelancer/earnings/transactions" className="text-xs font-black text-indigo-400 hover:text-indigo-300 transition uppercase tracking-wider">
              All Transactions &rarr;
            </Link>
          </div>
          <div className="overflow-x-auto">
            {transactions.length === 0 ? (
              <p className="text-slate-500 text-sm text-center py-12">No recent ledger transactions detected.</p>
            ) : (
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-slate-850/30 transition text-sm">
                      ...
                      <td className="py-4 px-6 font-mono text-xs text-slate-450 uppercase">
                        {tx.entry_type}
                      </td>
