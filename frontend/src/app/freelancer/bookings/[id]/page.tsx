    ...
    <div className="min-h-screen bg-background text-text-main py-12 px-4 md:px-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center">
          <Link href="/freelancer/bookings" className="text-xs text-primary font-extrabold uppercase tracking-wider hover:underline">
            ← Back to Bookings List
          </Link>
          <span className="text-[10px] text-text-muted font-mono font-bold">Reference ID: {booking.booking_number}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-surface border border-border-custom rounded-3xl p-6 md:p-8 shadow-xl space-y-4">
              <div className="flex justify-between items-start gap-4">
                <div>
                  ...
                  <p className="text-xs text-text-sub mt-2">Client User: {booking.client?.full_name}</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-cyan-400 mt-0.5">
                      Requested by: <strong className="text-text-main">{pendingReschedule.requested_by}</strong>
                    </p>
                    <p className="text-xs text-text-main mt-2">
                      Proposed Date: <strong className="text-text-main">{pendingReschedule.new_date} ({pendingReschedule.new_start_time.substring(0,5)} - {pendingReschedule.new_end_time.substring(0,5)})</strong>
                    </p>
                    {pendingReschedule.reason && (
                      <p className="text-[11px] text-text-sub italic mt-1">Reason: "{pendingReschedule.reason}"</p>
                    )}
                  </div>
                </div>
                ...
                {pendingReschedule.requested_by === "CLIENT" && (
                  <div className="flex gap-2 pt-2">
                    <button
                      ...
                      className="px-4 py-2 bg-primary hover:bg-primary-hover text-xs font-bold rounded-xl text-text-main transition"
                    >
                      Accept Reschedule
                    </button>
                    <button
                      ...
                      className="px-4 py-2 bg-background border border-border-custom text-text-sub text-xs font-bold rounded-xl hover:bg-surface-elevated transition"
                    >
                      Decline
                    </button>
                  </div>
            ...
            {showDecline && (
              <div className="bg-surface border border-border-custom rounded-3xl p-6 shadow-xl space-y-4">
                <h3 className="text-xs font-bold text-text-main uppercase tracking-wider">Decline Booking Request</h3>
                <form onSubmit={handleDeclineSubmit} className="space-y-4">
                  <div>
                    <label className="block text-[10px] text-text-sub uppercase font-black mb-1">Decline Reason (Optional)</label>
                    <textarea
                      rows={2}
            ...
            {showCancel && (
              <div className="bg-surface border border-border-custom rounded-3xl p-6 shadow-xl space-y-4">
                <h3 className="text-xs font-bold text-text-main uppercase tracking-wider">Cancel gig job</h3>
          ...
          <div className="space-y-6">
            ...
            {/* Financial Summary */}
            <div className="bg-surface border border-border-custom rounded-3xl p-6 shadow-xl space-y-4">
              <span className="text-[10px] text-primary font-extrabold uppercase tracking-wider block">Income details</span>
              
              <div className="flex justify-between items-center text-xs">
                <span className="text-text-sub">Total Income</span>
                ...
                {booking.status === "REQUESTED" && (
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="py-2 bg-primary hover:bg-primary-hover text-text-main text-xs font-bold rounded-xl transition uppercase tracking-wider"
                    >
                      Accept
                    </button>
                    <button
                      ...
                      className="py-2 bg-background border border-border-custom hover:bg-surface-elevated text-text-sub text-xs font-bold rounded-xl transition uppercase tracking-wider"
                    >
                      Decline
                    </button>
                  </div>
                ...
                {booking.status === "CONFIRMED" && (
                  <>
                    <button
                      disabled={actionLoading}
                      onClick={handleStart}
                      className="w-full py-2.5 bg-primary hover:bg-primary-hover text-text-main text-xs font-black rounded-xl transition uppercase tracking-wider animate-pulse"
                    >
                      Start Work Job
                    </button>
                    <button
                ...
                {booking.status === "IN_PROGRESS" && (
                  <>
                    <button
                      ...
                      className="w-full py-2.5 bg-primary hover:bg-primary-hover text-text-main text-xs font-black rounded-xl transition uppercase tracking-wider"
                    >
                      Deliver Output
                    </button>
                    <button
