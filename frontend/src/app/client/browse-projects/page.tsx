...
export default function ClientBrowseProjectsPage() {
  ...
  async function queryJobs() {
    try {
      ...
      setErrorMsg("Failed to query open project listings. Please try again later.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Container className="py-8">
      <div className="space-y-6">
        <PageHeader
          title="Browse Marketplace Projects"
          description="Discover open creative project postings available in the marketplace."
        />
        ...
        {loading ? (
          ...
          <EmptyState
            title="No open projects available."
            ...
            {jobs.map((job) => (
              ...
              >
                ...
                <div className="flex gap-3 mt-6 pt-4 border-t border-border-custom">
                  <div className="flex-grow py-2.5 text-center bg-surface border border-border-custom text-[11px] font-extrabold uppercase tracking-wider rounded-xl text-text-sub cursor-not-allowed transition flex items-center justify-center gap-1 shadow-sm">
                    View Only
                  </div>
                </div>
              </div>
